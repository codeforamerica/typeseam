...
class Provider(BaseProvider):
    ...
    def probation(self, answers):
        ...
        if on_probation:
            answers['textfield_15076739'] = ", ".join(
                [self.generator.city(), self.generator.state_abbr()])
            answers['textfield_15076767'] = self.generator.date_time_between(
                'now', '+5y').strftime('%b %Y')
        return answers
    ...
    def answers(self):
        answers = {
            "date_15075567": self.generator.date_time_between(
                '-70y', '-18y').strftime('%Y-%m-%d'),
            ...
            "textfield_15075504": self.random_element(
                [self.generator.first_name(), self.generator.last_name()]),
            "textfield_15075537": self.random_element(
                [self.generator.ssn(), ""]),
            "textfield_15075547": self.random_element(
                [self.bothify('#???????'), ""]),
            "textfield_15076804": self.random_int(max=4000),
            "textfield_15076814": self.random_int(max=2000),
            "textfield_15076818": self.random_element(
                [" ".join(self.generator.words(5)), ""]),
